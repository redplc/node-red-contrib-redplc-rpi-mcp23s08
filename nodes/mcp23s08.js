...
module.exports = function(RED) {
	...
	const sysmodule = syslib.LoadModule("rpi_mcp23s08.node");

	const MODE_INPUT       = "0";
	const MODE_INPUTPULLUP = "1";
	const MODE_OUTPUT      = "2";

	RED.nodes.registerType("mcp23s08", function (n) {
		var node = this;
		...
		if (!node.iserror) {
			...
			else if (!sysmodule.initDIO(node.devspi, node.devadr, parseInt(n.mode)))
